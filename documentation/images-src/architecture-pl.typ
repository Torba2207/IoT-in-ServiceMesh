...
#set text(font: "New Computer Modern", size: 9.5pt, lang: "pl")
...
#cetz.canvas(length: 1cm, {
  ...
  // czujniki
  node(1.4, 12.6, 2.2, 0.95, c-ext, [WS101 \ #text(7.5pt)[przycisk]])
  node(4.2, 12.6, 2.2, 0.95, c-ext, [WS202 \ #text(7.5pt)[PIR + światło]])
  node(7.0, 12.6, 2.2, 0.95, c-ext, [WS302 \ #text(7.5pt)[poziom dźwięku]])

  // brama
  node(4.2, 10.7, 3.4, 0.95, c-gw, [Milesight UG63 \ #text(7.5pt)[brama LoRaWAN]])
  ...
  content((4.9, 8.55), text(size: 8.5pt, fill: rgb("#3949ab"))[
    przestrzeń iot-system — siatka Linkerd: każde połączenie poniżej to mTLS, domyślnie blokada
  ])
  ...
  // główny potok
  node(LX, 5.9, 3.6, 0.95, c-brk, [Mosquitto \ #text(7.5pt)[broker MQTT]])
  node(LX, 4.3, 3.6, 0.95, c-app, [ChirpStack \ #text(7.5pt)[serwer LoRaWAN]])
  ...
  flow((LX, 3.82), (LX, 3.08), lbl: [zdekodowany uplink])
  flow((LX, 2.12), (RX - 1.4, 1.9))
  flow((RX - 1.4, 1.5), (LX, 0.95))
  flow((LX, 0.42), (LX, -0.32), lbl: [/api])

  flow((LX + 1.8, 4.3), (RX - 1.4, 4.3))
  flow((LX + 1.6, 4.0), (RX - 1.4, 2.0))
})
